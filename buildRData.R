...
downloadMissingImages<-function()
{
  library(data.table)
  BASE_URL<-"http://ifcb-data.whoi.edu/mvco/"
  IFCB<-as.data.table(readRDS('IFCB.RData'))
  download<-subset(IFCB,OriginalClass=='Other' & AutoClass == 'na')
  fileNames<-paste(download$Sample,"_",formatC(download$roi_number, width=5, flag="0"),".png",sep="")
  downloadURLs<-paste("http://ifcb-data.whoi.edu/mvco/",fileNames,sep="")
